...
/////////////////////////////////////////////////////////////////////////////
G2K2Pi4::G2K2Pi4(const std::string& name, ISvcLocator* pSvcLocator) :		//do not change this function
...
StatusCode G2K2Pi4::initialize(){
	...
	if(m_readSig==1)
	{
	        ...
	        //To store mc truth.
                ...
                pip1   = new TLorentzVector();
                pim1   = new TLorentzVector();
		kp   = new TLorentzVector();
                ...
                pip1_unfitted   = new TLorentzVector();
                pim1_unfitted   = new TLorentzVector();
    ...
    //Header
		...
		//Particles
                ...
                TreeAna->Branch("pip",&pip,32000,0);
                TreeAna->Branch("pim",&pim,32000,0);
                TreeAna->Branch("pip1",&pip1,32000,0);
                TreeAna->Branch("pim1",&pim1,32000,0);
		//Before 4c
                ...
                TreeAna->Branch("pip_unfitted",&pip_unfitted,32000,0);
                TreeAna->Branch("pim_unfitted",&pim_unfitted,32000,0);
		TreeAna->Branch("pip1_unfitted",&pip_unfitted,32000,0);
                TreeAna->Branch("pim1_unfitted",&pim_unfitted,32000,0);
...
//****************************************************
	...
	if((nGood != 6) || (nCharge != 0)) return StatusCode::SUCCESS;
...
//*********************************
	   ...
	   for(int i = 0; i < nGood; i++)
	   {
		...
		//identify kaon 
           if( (prob_pi>prob_e)&&(prob_pi>prob_k) ) {

                       if(mdcKalTrk->charge() > 0){
                              ipip.push_back(iGood[i]);
                                }
                       else{  ipim.push_back(iGood[i]);
                           }
...
//**************************************
         ...
         WTrackParameter wvpimTrk,wvpipTrk,wvpimTrk1,wvpipTrk1;
         ...
         pipTrk1->setPidType(RecMdcKalTrack::pion);
         wvpipTrk = WTrackParameter(mpi, pipTrk1->getZHelix(), pipTrk1->getZError());
        
         RecMdcKalTrack *pimTrk1 = (*(evtRecTrkCol->begin()+ipim[0]))->mdcKalTrack();
         pimTrk1->setPidType(RecMdcKalTrack::pion);
         wvpimTrk = WTrackParameter(mpi, pimTrk1->getZHelix(), pimTrk1->getZError());

	RecMdcKalTrack *pipTrk2 = (*(evtRecTrkCol->begin()+ipip[1]))->mdcKalTrack();
         pipTrk2->setPidType(RecMdcKalTrack::pion);
         wvpipTrk1 = WTrackParameter(mpi, pipTrk2->getZHelix(), pipTrk2->getZError());

         RecMdcKalTrack *pimTrk2 = (*(evtRecTrkCol->begin()+ipim[1]))->mdcKalTrack();
         pimTrk2->setPidType(RecMdcKalTrack::pion);
         wvpimTrk1 = WTrackParameter(mpi, pimTrk2->getZHelix(), pimTrk2->getZError());
	...
	WTrackParameter wpip1 = vtxfit->wtrk(4);
        WTrackParameter wpim1 = vtxfit->wtrk(5);
	...
	Chisq_low=1000;
        ...
        for(int i1 = 0; i1 < nGamma-1; i1++){
            ...
            //    kmfit->setEspread(ESpread);
                  ...
                  kmfit->AddTrack(4, wpip);
                  kmfit->AddTrack(5, wpim);
		 kmfit->AddTrack(6, wpip1);
                  kmfit->AddTrack(7, wpim1);
                  //kmfit->AddTrack(6, 0.0, gammaTrk3);
   		...
   		HepLorentzVector gamma1_ = kmfit->pfit(0);
                ...
                //HepLorentzVector gamma3_ = kmfit->pfit(6);
                HepLorentzVector kp_ = kmfit->pfit(2);   
                HepLorentzVector km_ = kmfit->pfit(3);   
                HepLorentzVector pip_ = kmfit->pfit(4);   
                HepLorentzVector pim_ = kmfit->pfit(5);   
		HepLorentzVector pip1_ = kmfit->pfit(6);
                HepLorentzVector pim1_ = kmfit->pfit(7);
                ...
                pip->SetPxPyPzE(pip_.px(), pip_.py(), pip_.pz(), pip_.e());
                pim->SetPxPyPzE(pim_.px(), pim_.py(), pim_.pz(), pim_.e());
		pip1->SetPxPyPzE(pip1_.px(), pip1_.py(), pip1_.pz(), pip1_.e());
                pim1->SetPxPyPzE(pim1_.px(), pim1_.py(), pim1_.pz(), pim1_.e());
                ...
                pip_unfitted->SetPxPyPzE((wpip.p()).px(), (wpip.p()).py(), (wpip.p()).pz(), (wpip.p()).e());
                pim_unfitted->SetPxPyPzE((wpim.p()).px(), (wpim.p()).py(), (wpim.p()).pz(), (wpim.p()).e());
		pip1_unfitted->SetPxPyPzE((wpip1.p()).px(), (wpip1.p()).py(), (wpip1.p()).pz(), (wpip1.p()).e());
                pim1_unfitted->SetPxPyPzE((wpim1.p()).px(), (wpim1.p()).py(), (wpim1.p()).pz(), (wpim1.p()).e());
